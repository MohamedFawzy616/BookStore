...
{
    public class PermanentArticle
    {
        public void Article_Currencies()
        {
            var db = new EgyptCurrencyRatesContext();

            var existArticles = db.Articles.Where(a => a.TypeId == 1 && a.Ispermanent ==true).ToList();
            db.Articles.RemoveRange(existArticles);
            db.SaveChanges();
            ...
            foreach (var currency in Currencies)
            {
                ...
                {
                    rates += string.Format("سعر شراء {0} في {1} {2} وسعر بيع {0} في {1} {3} . {4} &",
                                      rate.Currency.Name,
                                      rate.Bank.Name,
                                      rate.BuyPrice,
                ...
                {
                    Title = string.Format(@"أسعار {0} في البنوك المصرية اليوم", currency.Name),
                    Content = content,
                    Summary = summary,
                    Description = string.Format(@"تابع أسعار {0} في البنوك المصرية اليوم متابعة لحظة بلحظة سعر {0} في البنك الأهلي وسعر {0} في بنك مصر وسعر {0} في بنك الإسكندرية", currency.Name),
                    Keywords = string.Format(@"أسعار {0} في البنوك المصرية اليوم , أسعار {0} اليوم , {0} , سعر {0} اليوم , سعر {0} في البنك الأهلي , سعر {0} في بنك مصر , سعر {0} في بنك الإسكندرية", currency.Name),
                    Date = DateTime.Now,
        ...
        public void Article_Banks()
        {
            ...
            var existArticles = db.Articles.Where(a => a.TypeId == 2 && a.Ispermanent == true).ToList();
            db.Articles.RemoveRange(existArticles);
            db.SaveChanges();
...
", Verb_Article_Bank_1(), bank.Name, Analytics.Day(), Analytics.Date());
                ...
                {
                    _rate += string.Format("سعر شراء {0} في {1} {2} وسعر بيع {0} في {3} {4} , &", item.Currency.Name, item.Bank.Name, item.BuyPrice.ToString(), item.Bank.Name, item.SalePrice.ToString());
                ...
                Article article = new Article
                {
                    Title = string.Format("أسعار العملات في {0} اليوم", bank.Name),
                    Content = content,//{0}
                    Description = string.Format("أسعار العملات في {0} اليوم سعر الدولار في {0} وسعر اليورو في {0} وسعر الريال القطري في {0} وسعر الريال السعودي في {0} وسعر الدرهم الإماراتي في {0}", bank.Name),
                    Keywords = string.Format("أسعار العملات في {0} اليوم سعر الدولار في {0} , سعر اليورو في {0} , سعر الريال القطري في {0} , سعر الريال السعودي في {0} , سعر الدرهم الإماراتي في {0}", bank.Name),
                    Date = DateTime.Now,
        ...
        public void Article_CurrenciesInBanks()
        {
            ...
            var existArticles = db.Articles.Where(a => a.TypeId == 3 && a.Ispermanent == true).ToList();
            db.Articles.RemoveRange(existArticles);
            db.SaveChanges();
            ...
            foreach (var currency in Currencies)
            {
                foreach (var bank in db.Banks.Where(b => b.Visible == true && b.Id != 8 && b.Id != 9))
                {
                    int bankId = bank.Id;
                    ...
                    var Buy_Price = db.Rates.Include("Bank").Include("Currency").Where(r => r.CurrencyId == CurrencyId && r.BankId != 8 && r.BankId != 9).OrderByDescending(r => r.BuyPrice).FirstOrDefault();
                    var Sale_Price = db.Rates.Include("Bank").Include("Currency").Where(r => r.CurrencyId == CurrencyId && r.BankId != 8 && r.BankId != 9).OrderBy(r => r.SalePrice).FirstOrDefault();
                    ...
                    Article article = new Article()
                    {
                        Title = string.Format(@"سعر {0} في {1} اليوم", rate.Currency.Name, rate.Bank.Name, Analytics.Day(), Analytics.Date()),
        ...
        public void Article_Buy()
        {
            ...
            var existArticles = db.Articles.Where(a => a.TypeId == 4 && a.Ispermanent == true).ToList();
            db.Articles.RemoveRange(existArticles);
            db.SaveChanges();
            ...
            foreach (var currency in Currencies)
            {
                foreach (var bank in Banks)
                {
                    try
                    {
                        ...
                        Article article = new Article()
                        {
                            Title = string.Format("سعر شراء {0} في {1} اليوم", currency.Name, rate.Bank.Name),
                            Content = content,
        ...
        public void Article_Sale()
        {
            ...
            var existArticles = db.Articles.Where(a => a.TypeId == 5 && a.Ispermanent == true).ToList();
            db.Articles.RemoveRange(existArticles);
            db.SaveChanges();
            ...
            foreach (var currency in Currencies)
            {
                foreach (var bank in Banks)
                {
                    try
                    {
                        ...
                        Article article = new Article()
                        {
                            Title = string.Format("سعر بيع {0} في {1} اليوم", currency.Name, rate.Bank.Name),
                            Content = content,
                            Description = description,
                            Summary = summary,
                            Keywords = keywords,
